    ...
    public:
        ...
        void deallocate(void *pointer) {
            if (!try_deallocate(pointer)) {
                throw std::invalid_argument(
                    "memory_pool::sharded_fixed_block_pool deallocate received a pointer not owned by the pool");
            }
        }
        ...
        [[nodiscard]] pool_stats stats() const noexcept {
            ...
            for (const auto &shard: shards_) {
                ...
                total.deallocation_count = pool_stats::saturating_add(total.deallocation_count,
                                                                      current.deallocation_count);
